...
def verify_otp(request, otp_id):
    ...
    if request.method=='POST':
        ...
        if not otp_entered:
            otp_error = True
            return render(request, 'user_account/verify_otp.html',{'otp_error':otp_error, 'otp_id': otp_id})
        elif timezone.now() > expire_time:
            messages.error(request,"Otp Expired Please click resend otp for new otp")
            return render(request, 'user_account/verify_otp.html',{'otp_id': otp_id})
...
def verify_forgot_password_otp(request, otp_id):
    ...
    if request.method == 'POST':
        ...
        if not otp_entered:
            otp_error = True
            return render(request, 'user_account/verify_forgot_password_otp.html', {'otp_error': otp_error, 'otp_id': otp_id})
        elif timezone.now() > expire_time:
            messages.error(request, "OTP expired. Please request a new one.")
            return render(request, 'user_account/verify_forgot_password_otp.html',{'otp_id': otp_id})
        elif otp_entered == otp_obj.otp:
            otp_obj.delete()
...
def reset_password(request):
    ...
    return render(request, 'user_homepage/')
